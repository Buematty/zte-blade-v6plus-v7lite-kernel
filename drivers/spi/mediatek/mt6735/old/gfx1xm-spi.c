...
int gfx1xm_power_on(struct gfx1xm_dev *gfx1xm_dev, bool onoff)
{
	...
	if(onoff) {
		if (0 == gfx1xm_dev->poweron){
			...
			pinctrl_select_state(gfx1xm_dev->pc, gfx1xm_dev->irq_init);
			...
			/*power on*/
		 	pinctrl_select_state(gfx1xm_dev->pc, gfx1xm_dev->pwr_on);
			gfx1xm_dev->poweron = 1;
			msleep(20);

			/*INT GPIO set floating after poweron and controlled by GFX1XM*/
			pinctrl_select_state(gfx1xm_dev->pc, gfx1xm_dev->irq_in);
			msleep(1);
		...
		if (1 == gfx1xm_dev->poweron){
			pinctrl_select_state(gfx1xm_dev->pc, gfx1xm_dev->irq_dis); 
			msleep(10);
			pinctrl_select_state(gfx1xm_dev->pc, gfx1xm_dev->pwr_off);
...
static ssize_t gfx1xm_chipid_show(struct device *dev, 
	struct device_attribute *attr, char *buf)
{
	...
	//gfx1xm_spi_read_bytes(&gfx1xm, 0x4220, 10, reg_value);
	...
	printk("%s,9p chip version check pass!\n", __func__);
	ret = sprintf(buf, "0x02,0x08,0x90\n");
	return ret;

}
static DEVICE_ATTR(chipid, S_IRUGO, gfx1xm_chipid_show, NULL);

static ssize_t gfx1xm_updown_show(struct device *dev, 
	struct device_attribute *attr, char *buf)
{
	return (sprintf(buf, "%d", finger_updown));
}
static DEVICE_ATTR(finger_up_down, S_IRUGO, gfx1xm_updown_show, NULL);
...
static long gfx1xm_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case GFX1XM_IOC_CMD:
		...
		if ((ioc->cmd == GFX1XM_W) && (ioc->addr == GFX1XM_MODE_STATUS)){
			if (gfx1xm_dev->mode == GFX1XM_IMAGE_MODE)
...
//	spin_lock_irq(&gfx1xm_dev->spi_lock);
	if (1 == on) {//switch on esd
		if (0 == gfx1xm_dev->esd_running){
			...
			gfx1xm_debug(DEFAULT_DEBUG,"start esd work");
		...
		if (1 == gfx1xm_dev->esd_running) {
			...
			gfx1xm_debug(DEFAULT_DEBUG,"stop esd work");
		}
	}
...
{	
    ...
    gfx1xm_dev = container_of((struct delayed_work *)work, struct gfx1xm_dev, esd_check_work);   
	mutex_lock(&gfx1xm_dev->fb_lock);
...
**
*******************************************/
static irqreturn_t gfx1xm_irq(int irq, void *desc)
{
    irq_flag = 1;
...
static int gfx1xm_event_handler(void *para)
{
	...
	do{
	    ...
	    switch(mode){
		case GFX1XM_FF_MODE:
#if DEMO_APK //FF mode could get data
...
int gfx1xm_config_update(struct gfx1xm_dev *gfx1xm_dev)
{
	...
	if(chip_version_config==0x38){		
		if(!memcmp(buffer+GFX1XM_RDATA_OFFSET, vendor_id_1, 1)) {
			printk("gfx1xm: %s, read Vendor ID is 0x%x\n", __func__, buffer[GFX1XM_RDATA_OFFSET]);
			...
			printk("gfx1xm: Vendor ID not matched!!! no send 0x%x\n", buffer[GFX1XM_RDATA_OFFSET]);
			return 0;
			//memcpy(gfx1xm_dev->config + GFX1XM_WDATA_OFFSET, cfg8_info_group2, GFX1XM_CFG_LEN);
		}
	}else if(chip_version_config==0x36){
	...
	int id1 = -1;
	int id2 = -1;
	gpio_request(gfx1xm_dev->id_gpio, "fp_vendor_id");
	pinctrl_select_state(gfx1xm_dev->pc, gfx1xm_dev->id_up);
	id1= gpio_get_value(gfx1xm_dev->id_gpio);
	pinctrl_select_state(gfx1xm_dev->pc, gfx1xm_dev->id_down);
	id2= gpio_get_value(gfx1xm_dev->id_gpio);
	gfx1xm_debug(DEFAULT_DEBUG, "id = %lu id_up = %d, id_down = %d", gfx1xm_dev->id_gpio, id1, id2);
	gpio_free(gfx1xm_dev->id_gpio);
	
	if(chip_version_config==0x38){		
		if(id1 != id2) {
			printk("gfx1xm: %s, send kaier config\n", __func__);
			memcpy(gfx1xm_dev->config + GFX1XM_WDATA_OFFSET, cfg8_info_group1, GFX1XM_CFG_LEN);
		} else if (id1 == 1) {
			printk("gfx1xm: %s, send helitai config\n", __func__);
			memcpy(gfx1xm_dev->config + GFX1XM_WDATA_OFFSET, cfg8_info_group0, GFX1XM_CFG_LEN);
		} else if (id1 == 0) {
			printk("gfx1xm: %s, send syx config\n", __func__);
			memcpy(gfx1xm_dev->config + GFX1XM_WDATA_OFFSET, cfg8_info_group2, GFX1XM_CFG_LEN);
		} else {	//default config is kaier
			printk("gfx1xm: Vendor ID not matched!!!  send kaier\n");
			memcpy(gfx1xm_dev->config + GFX1XM_WDATA_OFFSET, cfg8_info_group1, GFX1XM_CFG_LEN);
		}
	}
#endif
...
static int  gfx1xm_probe(struct spi_device *spi)
{
...
#endif
	FUNC_ENTRY();
	printk("gfx1x driver version is %s.\n",GFX1XM_DRIVER_VERSION);
	...
	gfx1xm_debug(DEFAULT_DEBUG, "pinctl status ok?");
	/*power on*/
	...
	if(node) {
		...
		gfx1xm_dev->spi->irq =  irq_of_parse_and_map(node, 0);
		gfx1xm_debug(DEFAULT_DEBUG,"irq number %d", gfx1xm_dev->spi->irq);		
		if (!gfx1xm_dev->spi->irq) {
			gfx1xm_error("Failed to get irq number.");
			goto err_get_irq;
		}
		...
		if (request_irq(gfx1xm_dev->spi->irq, gfx1xm_irq, IRQF_TRIGGER_RISING, "gf-eint", NULL)) {
            gfx1xm_error("IRQ LINE NOT AVAILABLE!!\n");
            goto err_get_irq;
    	}  
...
static int fp_probe(struct platform_device *pdev)
{
	...
	gfx1xm_dev->pc = devm_pinctrl_get(&pdev->dev);
    if (IS_ERR(gfx1xm_dev->pc)) {
        gfx1xm_error("get pinctrl config failed");
        return PTR_ERR(gfx1xm_dev->pc);
    }

	gfx1xm_dev->pwr_on = pinctrl_lookup_state(gfx1xm_dev->pc, "pwr_on");
	gfx1xm_dev->pwr_off = pinctrl_lookup_state(gfx1xm_dev->pc, "pwr_off");
	gfx1xm_dev->rst_low = pinctrl_lookup_state(gfx1xm_dev->pc, "rst_low");
	gfx1xm_dev->rst_high = pinctrl_lookup_state(gfx1xm_dev->pc, "rst_high");
	gfx1xm_dev->irq_init = pinctrl_lookup_state(gfx1xm_dev->pc, "irq_init");
	gfx1xm_dev->irq_in = pinctrl_lookup_state(gfx1xm_dev->pc, "irq_in");
	gfx1xm_dev->irq_dis = pinctrl_lookup_state(gfx1xm_dev->pc, "irq_dis");
	gfx1xm_dev->irq_en = pinctrl_lookup_state(gfx1xm_dev->pc, "irq_en");
	gfx1xm_dev->id_up = pinctrl_lookup_state(gfx1xm_dev->pc, "id_up");
	gfx1xm_dev->id_down = pinctrl_lookup_state(gfx1xm_dev->pc, "id_down");

	if (IS_ERR(gfx1xm_dev->pwr_on) || IS_ERR(gfx1xm_dev->pwr_off)
		 || IS_ERR(gfx1xm_dev->rst_low) || IS_ERR(gfx1xm_dev->rst_high) || IS_ERR(gfx1xm_dev->irq_init)
		 || IS_ERR(gfx1xm_dev->irq_in) || IS_ERR(gfx1xm_dev->irq_dis) || IS_ERR(gfx1xm_dev->irq_en)
		 || IS_ERR(gfx1xm_dev->id_up) || IS_ERR(gfx1xm_dev->id_down)) {
		gfx1xm_error("lookup pinctrl state failed");
		return -1;
	}

	gfx1xm_dev->id_gpio = of_get_gpio(pdev->dev.of_node, 0);
	gfx1xm_debug(DEFAULT_DEBUG, "platform_driver probe success");
	return 0;
}
...
static int __init gfx1xm_init(void)
{
	...
	if (platform_driver_register(&fp_driver) != 0){
		gfx1xm_error("Failed to register SPI platform driver.");
	}
