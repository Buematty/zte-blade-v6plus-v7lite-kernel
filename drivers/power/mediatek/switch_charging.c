...
static void set_jeita_charging_current(void)
{
	...
	if (g_temp_status == TEMP_POS_0_TO_POS_10) {
		...
		battery_log(BAT_LOG_CRTI, "[BATTERY] JEITA 0to 10 set charging current : %d\r\n",
			    g_temp_CC_value);
	}
	else if (g_temp_status == TEMP_POS_45_TO_POS_60) {
		g_temp_CC_value = CHARGE_CURRENT_500_00_MA;
		g_temp_input_CC_value = CHARGE_CURRENT_1000_00_MA;
		battery_log(BAT_LOG_CRTI, "[BATTERY] JEITA 45to60 set charging current : %d\r\n",
			    g_temp_CC_value);
	}
