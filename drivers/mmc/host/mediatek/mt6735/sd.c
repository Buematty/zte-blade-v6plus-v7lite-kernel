...
#endif
#define GPIO_SD_POWER_EN     76 //zhangjian add for SD card hot plug
...
//modified by zhangjian start to merge the MTK modification to solve the issue that sim card card be burned with low possibility when unplug sdcard
void msdc_sd_power_off_quick(void)
{
   ...
   gpio_set_value(GPIO_SD_POWER_EN, 0);
   #endif
}
//modified  end 
...
static void msdc_dma_start(struct msdc_host *host)
{
	...
	if (host->data && host->data->flags & MMC_DATA_WRITE) {
		host->write_timeout_ms = min_t(u32, max_t(u32, host->data->blocks * 500,
...
static int msdc_ops_get_cd(struct mmc_host *mmc)
{
	...
	pr_debug("Card insert<%d> Block bad card<%d>\n", host->card_inserted,
      ...
      //zhangjian add for SD card hot plug
      #ifdef CONFIG_P635A32
      if (host->card_inserted )
      {   
      printk("SD power on\n") ;              
      gpio_set_value(GPIO_SD_POWER_EN, 1);       
      }
      else
      {
      printk("SD power off\n") ;           
