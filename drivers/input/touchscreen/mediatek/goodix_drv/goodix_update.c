...
#ifdef CONFIG_CREATE_SYS_INTERFACE
#define TPD_FW_READY			  0x04
static u8 *gtp_tpd_FW = NULL;
#endif
...
static u8 gup_enter_update_judge(st_fw_head *fw_head)
{
			...
			(!memcmp(fw_head->pid, "91XX", 4)))
	{
		...
		else
		{
         ...
         GTP_INFO("update_msg.ic_fw_msg.vid is 0x%x \n",update_msg.ic_fw_msg.vid);
		 GTP_INFO("fw_head->vid is 0x%x \n",fw_head->vid);	  
		//The third condition
...
#endif
#ifdef CONFIG_CREATE_SYS_INTERFACE
	if (got_file_flag == TPD_FW_READY) {
		if(gtp_tpd_FW == NULL) return FAIL;
		if(SEEK_SET == set_or_end) {
			memcpy(buf, &gtp_tpd_FW[FW_HEAD_LENGTH + offset], length);
		} else{		//seek end
			memcpy(buf, &gtp_tpd_FW[update_msg.fw_total_len + FW_HEAD_LENGTH - offset], length);
		}
		return SUCCESS;
	}
#endif
...
u8 tpd_fw_update_judge(st_fw_head *fw_head, int force_upg)
{
	u16 u16_tmp;
	s32 i = 0;
	u32 fw_len = 0;
	...
	TPD_DMESG("FILE HARDWARE INFO:%02x%02x%02x%02x\n", fw_head->hw_info[0], fw_head->hw_info[1], fw_head->hw_info[2], fw_head->hw_info[3]);
	TPD_DMESG("FILE PID:%s\n", fw_head->pid);
	TPD_DMESG("FILE VID:%04x\n", fw_head->vid);
	TPD_DMESG("IC HARDWARE INFO:%02x%02x%02x%02x\n", update_msg.ic_fw_msg.hw_info[0], update_msg.ic_fw_msg.hw_info[1],
			 update_msg.ic_fw_msg.hw_info[2], update_msg.ic_fw_msg.hw_info[3]);
	TPD_DMESG("IC PID:%s\n", update_msg.ic_fw_msg.pid);
	TPD_DMESG("IC VID:%04x\n", update_msg.ic_fw_msg.vid);
	
	pid_cmp_len = strlen(fw_head->pid);
	memcpy(buf, update_msg.ic_fw_msg.pid, pid_cmp_len);

	if (!memcmp(fw_head->pid, "9158", 4) && !memcmp(update_msg.ic_fw_msg.pid, "915S", 4)) {
		TPD_DMESG("Update GT915S to GT9158 directly!\n");
		return SUCCESS;
	}
	...
	if (update_msg.fw_total_len != fw_len) {
		TPD_DMESG("Inconsistent firmware size, Update aborted! Default size: %d(%dK), actual size: %d(%dK)\n", fw_len, fw_len/1024, update_msg.fw_total_len, update_msg.fw_total_len/1024);
		return FAIL;
	}
	TPD_DMESG("Firmware length:%d(%dK)\n", update_msg.fw_total_len, update_msg.fw_total_len/1024);
	
	if (update_msg.force_update != 0xBE) {
		TPD_DMESG("FW chksum error,need enter update.\n");
		return SUCCESS;
	}
	
	// 20130523 start
	if (strlen(update_msg.ic_fw_msg.pid) < 3) {
		TPD_DMESG("Illegal IC pid, need enter update\n");
		return SUCCESS;
	} else {
		for (i = 0; i < 3; i++)
		{
			if ((update_msg.ic_fw_msg.pid[i] < 0x30) || (update_msg.ic_fw_msg.pid[i] > 0x39)) {
				TPD_DMESG("Illegal IC pid, out of bound, need enter update\n");
				return SUCCESS;
			}
		}
	}
	...
	{
		if(!memcmp(fw_head->pid, "91XX", 4)) {
			TPD_DMESG("Force none same pid update mode.\n");
		} else {
			TPD_DMESG("Get the same pid.\n");
		}

		//The third condition
		if (fw_head->vid > update_msg.ic_fw_msg.vid) {
			TPD_DMESG("Need enter update.\n");
			return SUCCESS;
		}
		TPD_DMESG("Don't meet the third condition.\n");
		TPD_DMESG("File VID <= Ic VID, update aborted!\n");
	} else {
		TPD_DMESG("File PID != Ic PID, update aborted!\n");
	}
...
int tpd_goodix_compare_fw(struct tpd_classdev_t *cdev, unsigned char * data, unsigned int size, int force_upg)
{
	s32 retval = 0;
	s32 i = 0;
	s32 fw_checksum = 0;
	...
	TPD_DMESG("Update by default firmware array\n");
	gtp_tpd_FW = (u8*)data;
	update_msg.fw_total_len = size - FW_HEAD_LENGTH;
	if (size < (FW_HEAD_LENGTH+FW_SECTION_LENGTH*4+FW_DSP_ISP_LENGTH+FW_DSP_LENGTH+FW_BOOT_LENGTH))
	{
		TPD_DMESG("INVALID gtp_default_FW, check your gt9xx_firmware.h file!\n");
		retval = 0xff;
		goto out;		   
	}
	TPD_DMESG("Firmware actual size: %d(%dK)\n", update_msg.fw_total_len, update_msg.fw_total_len/1024);
	memcpy(&fw_head, &data[0], FW_HEAD_LENGTH);

	//check firmware legality
	fw_checksum = 0;
	for(i=0; i< update_msg.fw_total_len; i+=2) {
		fw_checksum += (data[FW_HEAD_LENGTH + i] << 8) + data[FW_HEAD_LENGTH + i + 1];
	}

	TPD_DMESG("firmware checksum:%x\n", fw_checksum&0xFFFF);
	if (fw_checksum&0xFFFF) {
		TPD_DMESG("Illegal firmware file.\n");
		retval = 0xff;
		goto out;
	}
	got_file_flag = TPD_FW_READY;
	retval = gup_get_ic_fw_msg(i2c_client_point);
	if(FAIL == retval) {
		TPD_DMESG("[update_proc]get ic message fail.\n");
		retval = 0xff;
		goto out;
	}
	
	retval = tpd_fw_update_judge(&fw_head, force_upg);
	if(FAIL == retval && force_upg == 0) {
		TPD_DMESG("[update_proc]Check *.bin file fail.\n");
		retval = 0xff;
	} else {
		retval = 0;
	}
...
int tpd_goodix_enter_update(unsigned char * data, unsigned int size)
{
	s32 ret = 0;
	u8	retry = 0;
	s32 update_ret = FAIL;
...
int tpd_goodix_update_proc(struct tpd_classdev_t *cdev, unsigned char * data, unsigned int size)
{
	int retval = -1;
	
	gtp_tpd_FW = (u8*)data;
	retval = tpd_goodix_enter_update(data, size);
	if(retval == SUCCESS) {
		retval = 0;
	} else {
		retval = -1;
