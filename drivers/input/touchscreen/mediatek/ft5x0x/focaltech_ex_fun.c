...
static struct kobject *mtk_tp_wake_gesture_kobj;
...
/**********zhangzhao add for power down charge*************/
extern  ssize_t fts_powerdown_charge_show(struct device *dev, struct device_attribute *attr, char *buf);
extern ssize_t fts_powerdown_charge_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count);
/**********zhangzhao add for power down charge*************/

static DEVICE_ATTR(powerdown_charger,  S_IRUGO|S_IWUSR|S_IWGRP,  fts_powerdown_charge_show,  fts_powerdown_charge_store); 
/* add your attr in here*/
static struct attribute *wake_gesture_attributes[] = {
	&dev_attr_wake_gesture.attr,
	&dev_attr_powerdown_charger.attr,//	zhangzhao add for power down charge
	NULL
};

static struct attribute_group wake_gesture_attribute_group = {
	.attrs = wake_gesture_attributes
};
...
* Return: success =0
***********************************************************************/
int fts_create_wake_gesture_sysfs(struct i2c_client * client)
{
    int err;
    mtk_tp_wake_gesture_kobj = kobject_create_and_add("tp_wake_gesture", NULL);
    if (!mtk_tp_wake_gesture_kobj)
    {
        err = -EINVAL;
        dev_err(&client->dev, "%s() - ERROR Unable to create shtsc_wake_gesture_kobj\n",__func__);
    }
    err = sysfs_create_group(mtk_tp_wake_gesture_kobj, &wake_gesture_attribute_group);
    if (0 != err) 
    {
        dev_err(&client->dev, "%s() - ERROR: sysfs_create_group() failed.\n", __func__);
        sysfs_remove_group(&client->dev.kobj, &wake_gesture_attribute_group);
        kobject_put(mtk_tp_wake_gesture_kobj);
        return -EIO;
    } 
    else 
    {
...
* Return: no
***********************************************************************/
int fts_remove_wake_gesture_sysfs(struct i2c_client * client)
{
	sysfs_remove_group(&client->dev.kobj, &wake_gesture_attribute_group);
	kobject_put(mtk_tp_wake_gesture_kobj);
	return 0;
