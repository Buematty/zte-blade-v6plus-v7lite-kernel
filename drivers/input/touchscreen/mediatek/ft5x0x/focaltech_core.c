...
bool enable_wakeup_gesture = false;
bool enter_wakeup_gesture = false;
//add end
...
unsigned int tpd_int_gpio_number = 10;
...
static int touch_event_handler(void *unused)
{
	...
	do {
		...
		set_current_state(TASK_RUNNING);
              ...
              #if FTS_GESTRUE_EN
              if (enter_wakeup_gesture) //zhangjian add if (enable_wakeup_gesture)
              {
                  printk("[FTS][Touch] read  gestrue value start \n");
...
static ssize_t fts_tpd_read(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
    ...
    u8 fts_fm_ver = 0;
    u8 vendor_id = 0;
    fts_read_reg(fts_i2c_client, FTS_REG_VENDOR_ID, &vendor_id);
    fts_read_reg(fts_i2c_client, FTS_REG_FW_VER, &fts_fm_ver);
    switch (vendor_id)
    {    
        case 0xda:	   
            len += sprintf(buffer_tpd+len, "TP module: lide (0xda)\n"); //some lide TP module is 0xda
            break;
        case 0x60:	   
            len += sprintf(buffer_tpd+len, "TP module: lide (0x60)\n");
            break;
        case 0x67:	   
            len += sprintf(buffer_tpd+len, "TP module: dijing (0x67)\n");
            break;        
        default:
            len += sprintf(buffer_tpd+len, "vendor id is 0x%x \n",vendor_id);
            break;  
    }   
...
/*********88zhangzhao add for power down charge************/
 ssize_t fts_powerdown_charge_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%u,\n", enable_wakeup_gesture);
}


 ssize_t fts_powerdown_charge_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    unsigned int input;
...
*/
	...
	#endif
       fts_create_wake_gesture_sysfs(fts_i2c_client);//zhangjian add for gesture
       fts_create_proc(); //zhangjian add for adb proc read
	//hidi2c_to_stdi2c(fts_i2c_client);
       #if 0 //zhangjian modify for FW upgrade
       fts_get_upgrade_array();
       #else
       fts_get_upgrade_array_CTPM_FW();
       #endif
	#ifdef FTS_CTL_IIC
...
static void tpd_resume(struct device *h)
{
	...
	msleep(200);
       ...
       #if FTS_GESTRUE_EN
       if (enable_wakeup_gesture) //zhangjian add if (enable_wakeup_gesture)
       {
	...
	fts_write_reg(i2c_client, FT_GESTRUE_MODE_SWITCH_REG, data);
      	...
      	enable_irq(fts_touch_irq);
	#else
       if ( enter_wakeup_gesture)
       {
           enter_wakeup_gesture = false;//zhangjian add for gesture
           printk("fts resume disable_irq_wake\n");
           disable_irq_wake(fts_touch_irq);
       }
       else
       {
           printk("fts resume enable_irq\n");
           enable_irq(fts_touch_irq);
       }
       #endif
#endif
    ...
    {           
           ...
           for(i = 0; i < 10; i++)
           {
               ...
               if(state == 1)
               {
                   ...
                   enter_wakeup_gesture = true;	 
                   enable_irq_wake(fts_touch_irq);
                   return;
               }
               else
               {
...
#else
       if ( !enter_wakeup_gesture) //zhangjian add if ( !enter_wakeup_gesture)
       {
           printk("fts suspend disable_irq\n");
           disable_irq(fts_touch_irq);
