...
static unsigned char CTPM_FW[] = {
	#include "FT3427_dijing_app.i"
};
#else //you can add other FW ,here if need
static unsigned char CTPM_FW_lide[] = {
	#include "FT3427_lide_app.i"
};
static unsigned char CTPM_FW_dijing[] = {
	#include "FT3427_dijing_app.i"
};
static unsigned char *CTPM_FW;
int	ctpfw_filenth =0;
bool FW_ERR_FORCE_UPDATE = false;
...
int fts_5x36_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	u8  	bt_ecc;
	int	i_ret;
	int	fw_filenth = ctpfw_filenth;//zhangjian modify sizeof(CTPM_FW);
...
//zhangjian add for FW upgrade
int fts_5x46_read_para_from_bootloader(struct i2c_client *client, u8 *pVendorID, u8 *pChipID)
{
	u8 reg_val[2] = {0};
	u32 i = 0;
	u8 auc_i2c_write_buf[10];
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		i_ret = fts_write_reg(client, FTS_RST_CMD_REG1, FTS_UPGRADE_AA);
		if(i_ret < 0)
			TPD_DEBUG("[FTS] failed writing  0xAA to register 0xfc! \n");
		msleep(10);
		
		/*write 0x55 to register 0xfc*/
		i_ret = fts_write_reg(client, FTS_RST_CMD_REG1, FTS_UPGRADE_55);
		if(i_ret < 0)
			TPD_DEBUG("[FTS] failed writing  0x55 to register 0xfc! \n");
		msleep(200);
		...
		i_ret = fts_i2c_write(client, auc_i2c_write_buf, 2);
		if(i_ret < 0)
			TPD_DEBUG("[FTS] failed enter into upgrade mode!n");
...
int FTS_read_chipid_vendorid(struct i2c_client *client, u8 *VendorID, u8 *ChipID)
{  
    u8 retry=0;
    while(retry++ < 5)
    {
        fts_read_reg(fts_i2c_client, FTS_REG_ID,ChipID);
        fts_read_reg(fts_i2c_client, FTS_REG_FW_VENDOR_ID,VendorID);
    if ((*ChipID != 0)&&(*VendorID != 0))
    {		
        printk("%s chip_id = %x,vendor_id =%x\n", __func__, *ChipID,*VendorID);
        return 0;
    }
        printk("FTS: read chipid vendorid failed time %d \n.",retry);
        msleep(10);
    }
    return -EIO;;
}

void fts_get_upgrade_array_CTPM_FW(void)
{
    u8 chip_id,vendor_id;
    u32 i;
    int ret = 0;
    ret =  FTS_read_chipid_vendorid(fts_i2c_client, &vendor_id,&chip_id);
    if (ret<0)
    {
        printk("FW upgrade may fail before,read para from bootloader.\n");
        ret = fts_5x46_read_para_from_bootloader(fts_i2c_client, &vendor_id,&chip_id);
        FW_ERR_FORCE_UPDATE = true;
       if (ret<0) 
       {
           printk("read para from bootloader fail");
       }
    }
    if ((vendor_id != 0x60) && (vendor_id != 0x67))
    {
        chip_id = 0x54;
        vendor_id =  lcd_read_vendor_id;
        FW_ERR_FORCE_UPDATE = true;
    }
    printk("%s chip_id = %x,vendor_id =%x\n", __func__, chip_id,vendor_id);
    ...
    switch (vendor_id){   
    case 0x60:
        printk("[FTS] vendor  is lide.\n");	
        CTPM_FW = CTPM_FW_lide;
        ctpfw_filenth = sizeof(CTPM_FW_lide);
        break;
    case 0x67:
        printk("[FTS] vendor  is dijing.\n");	
        CTPM_FW = CTPM_FW_dijing;
        ctpfw_filenth = sizeof(CTPM_FW_dijing);
        break;
    default:	
        printk("[FTS] vendor  is error.\n");        
        break;	
    }
...
{
	u16 ui_sz;
	ui_sz = ctpfw_filenth;//zhangjian modify sizeof(CTPM_FW);
	if (ui_sz > 2)
	{
...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
	u8 *pbt_buf = NULL;
	int i_ret=0;
	int fw_len = ctpfw_filenth;//zhangjian modify sizeof(CTPM_FW);
	...
	*/
		...
		||(fts_updateinfo_curr.CHIP_ID==0x55) ||(fts_updateinfo_curr.CHIP_ID==0x06) ||(fts_updateinfo_curr.CHIP_ID==0x0a) ||(fts_updateinfo_curr.CHIP_ID==0x08))
	{
			...
			&& (CTPM_FW[fw_len - 3] ^ CTPM_FW[fw_len - 4]) == 0xFF) 
		{
			...
			if ((fts_updateinfo_curr.CHIP_ID==0x55) ||(fts_updateinfo_curr.CHIP_ID==0x08) ||(fts_updateinfo_curr.CHIP_ID==0x0a))
			{
				i_ret = fts_5x06_ctpm_fw_upgrade(client, pbt_buf, ctpfw_filenth);//zhangjian modify ctpfw_filenth
			}
			else if ((fts_updateinfo_curr.CHIP_ID==0x11) ||(fts_updateinfo_curr.CHIP_ID==0x12) ||(fts_updateinfo_curr.CHIP_ID==0x13) ||(fts_updateinfo_curr.CHIP_ID==0x14))
			{
				i_ret = fts_5x36_ctpm_fw_upgrade(client, pbt_buf, ctpfw_filenth); //zhangjian modify ctpfw_filenth
			}
			else if ((fts_updateinfo_curr.CHIP_ID==0x06))
			{
				i_ret = fts_6x06_ctpm_fw_upgrade(client, pbt_buf, ctpfw_filenth); //zhangjian modify ctpfw_filenth
			}
			if (i_ret != 0)
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x36)||(fts_updateinfo_curr.CHIP_ID==0xe)) //zhangjian add ts_updateinfo_curr.CHIP_ID==0xe
	{
		if (fw_len < 8 || fw_len > 32 * 1024) 
		{
			dev_err(&client->dev, "%s:FW length error\n", __func__);
		...
		pbt_buf = CTPM_FW;
		i_ret = fts_6x36_ctpm_fw_upgrade(client, pbt_buf, ctpfw_filenth);//zhangjian modify ctpfw_filenth
		if (i_ret != 0)
			dev_err(&client->dev, "%s:upgrade failed. err.\n",__func__);
	}
	else if ((fts_updateinfo_curr.CHIP_ID==0x64))
	{
		...
		pbt_buf = CTPM_FW;
		i_ret = fts_6336GU_ctpm_fw_upgrade(client, pbt_buf, ctpfw_filenth);//zhangjian modify sizeof(CTPM_FW););
		if (i_ret != 0)
			dev_err(&client->dev, "%s:upgrade failed. err.\n",__func__);
	}
	else if ((fts_updateinfo_curr.CHIP_ID==0x54))
	{
		...
		pbt_buf = CTPM_FW;
		/*call the upgrade function*/
		i_ret = fts_5x46_ctpm_fw_upgrade(client, pbt_buf, ctpfw_filenth); //zhangjian modify ctpfw_filenth
		if (i_ret != 0)
		{
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x58))
	{
		...
		pbt_buf = CTPM_FW;
		/*call the upgrade function*/
		i_ret = fts_5822_ctpm_fw_upgrade(client, pbt_buf, ctpfw_filenth); //zhangjian modify ctpfw_filenth
		if (i_ret != 0) 
		{
			dev_err(&client->dev, "[FTS] upgrade failed. err=%d.\n", i_ret);
		} 
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x59))
	{
	    	if (fw_len < 8 || fw_len > 54*1024) 
		{
    		...
    		pbt_buf = CTPM_FW;
	   	 /*call the upgrade function*/
    		i_ret = fts_5x26_ctpm_fw_upgrade(client, pbt_buf, ctpfw_filenth); //zhangjian modify ctpfw_filenth
    		if (i_ret != 0) 
		{
    			dev_err(&client->dev, "[FTS] upgrade failed. err=%d.\n", i_ret);
    		} 
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x86))
	{
		...
		i_ret =  fts_8606_ctpm_fw_upgrade(client, pbt_buf, ctpfw_filenth);//zhangjian modify sizeof(CTPM_FW);
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x87))
	{
		...
		i_ret =  fts_8716_ctpm_fw_upgrade(client, pbt_buf, ctpfw_filenth);//zhangjian modify sizeof(CTPM_FW);
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x0E))
	{
		...
		pbt_buf = CTPM_FW;
		i_ret = fts_3x07_ctpm_fw_upgrade(client, pbt_buf, ctpfw_filenth);//zhangjian modify sizeof(CTPM_FW);
		if (i_ret != 0)
			dev_err(&client->dev, "%s:upgrade failed. err.\n",__func__);
	}
	return i_ret;
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	printk("[FTS] uc_tp_fm_ver is 0x%x.\n",uc_tp_fm_ver);
	uc_host_fm_ver = fts_ctpm_get_i_file_ver();
	printk("uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",uc_tp_fm_ver, uc_host_fm_ver);
	#if 0//zhangjian modify
	if (uc_tp_fm_ver == FTS_REG_FW_VER ||	uc_tp_fm_ver < uc_host_fm_ver )
	#else
	if (true == FW_ERR_FORCE_UPDATE ||	uc_tp_fm_ver < uc_host_fm_ver )
	#endif
	{
        FW_ERR_FORCE_UPDATE = false;//zhangjian add
		msleep(100);
