...
s32 gt1x_read_version(struct gt1x_version_info *ver_info)
{
	...
	ret = gpio_request(57, NULL);
	if (ret) {
		GTP_ERROR("Could not request GPIO#57.\n");
		/*return -EIO;*/
	}
		ret = gpio_request(58, NULL);
	if (ret) {
		GTP_ERROR("Could not request GPIO#58.\n");
		/*return -EIO;*/
	}

	ret = gpio_direction_input(57);
	if (ret) {
		GTP_ERROR("Could not set GPIO#57 as input.\n");
		/*return -EIO;*/
	}
		ret = gpio_direction_input(58);
	if (ret) {
		GTP_ERROR("Could not set GPIO#58 as input.\n");
		/*return -EIO;*/
	}


	pin_lcd_id0= gpio_get_value(58);
	pin_lcd_id1= gpio_get_value(57);
	GTP_INFO("PIN0 = %d,pin1=%d",pin_lcd_id0,pin_lcd_id1);
