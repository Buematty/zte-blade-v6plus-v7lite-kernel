...
static struct kobject *mtk_gt1k_tp_wake_gesture_kobj;
...
struct proc_dir_entry *mt_tsc_entry = NULL;

ssize_t gt1x_tsc_read(struct file *file, char __user *page, size_t size, loff_t *ppos)

{
	...
	len += sprintf(page + len, "Firmware ver  : V%x \n", gt1x_version.patch_id);


	gt1x_i2c_read(0x8050,buf,1);
	len += sprintf(page + len, "Cfg ver  : V%x\n", buf[0]);
	...
	*ppos += len;
    ...
    static const struct file_operations tsc_ops = {
        .owner = THIS_MODULE,
        .read = gt1x_tsc_read,
        .write = NULL,
    };
...
static ssize_t gt1k_wake_gesture_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	s32 ret = -1;

	GTP_DEBUG("visit gt1x_gesture_data_read. ppos");
...
static ssize_t gt1k_wake_gesture_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	s32 ret = 0;
    unsigned int input;
...
/*   if(gesture_enabled ==1)
	memset(gestures_flag, 1, sizeof(gestures_flag));
    else
	memset(gestures_flag, 0, sizeof(gestures_flag));
*/	
	GTP_DEBUG("gesture enabled:%x, %d ret:%d ", gesture_enabled,(is_all_dead(gestures_flag, sizeof(gestures_flag))), ret);

	return count;
}

static DEVICE_ATTR(wake_gesture,  S_IRUGO|S_IWUSR|S_IWGRP,  gt1k_wake_gesture_show,  gt1k_wake_gesture_store); 
/* add your attr in here*/
static struct attribute *gt1k_wake_gesture_attributes[] = {
	&dev_attr_wake_gesture.attr,	
	NULL
};

static struct attribute_group gt1k_wake_gesture_attribute_group = {
	.attrs = gt1k_wake_gesture_attributes
};

/************************************************************************
* Name: fts_create_wake_gesture_sysfs
* Brief:  create sysfs for debug
* Input: i2c info
* Output: nosus
* Return: success =0
***********************************************************************/
int gt1k_create_wake_gesture_sysfs(struct i2c_client * client)
{
    int err;
    mtk_gt1k_tp_wake_gesture_kobj = kobject_create_and_add("tp_wake_gesture", NULL);
    if (!mtk_gt1k_tp_wake_gesture_kobj)
    {
        err = -EINVAL;
        dev_err(&client->dev, "%s() - ERROR Unable to create shtsc_wake_gesture_kobj\n",__func__);
    }
    err = sysfs_create_group(mtk_gt1k_tp_wake_gesture_kobj, &gt1k_wake_gesture_attribute_group);
    if (0 != err) 
    {
        dev_err(&client->dev, "%s() - ERROR: sysfs_create_group() failed.\n", __func__);
        sysfs_remove_group(&client->dev.kobj, &gt1k_wake_gesture_attribute_group);
        kobject_put(mtk_gt1k_tp_wake_gesture_kobj);
        return -EIO;
    } 
    else 
    {
...
/************************************************************************
* Name: fts_remove_wake_gesture_sysfs
* Brief:  remove sys
* Input: i2c info
* Output: no
* Return: no
***********************************************************************/
int gt1k_remove_wake_gesture_sysfs(struct i2c_client * client)
{
	sysfs_remove_group(&client->dev.kobj, &gt1k_wake_gesture_attribute_group);
	kobject_put(mtk_gt1k_tp_wake_gesture_kobj);
	return 0;
...
int gesture_enter_doze(void)
{
	...
	while (retry++ < 5) {
		if (!gt1x_send_cmd(0x08, 0)) {
			GTP_INFO("entering doze mode write reg complete");
...
s32 gesture_event_handler(struct input_dev *dev)
{
	...
	GTP_INFO("gesture_doze_status = %d\n",gesture_doze_status);
	if (DOZE_ENABLED == gesture_doze_status) {
		...
		/*	if (!QUERYBIT(gestures_flag, doze_buf[0])) {
			...
			if((doze_buf[0] =='c')||(doze_buf[0] =='w')||(doze_buf[0] =='o')||
				(doze_buf[0] =='m')||(doze_buf[0] ==0x5E)||(doze_buf[0] ==0xAA)||(doze_buf[0] ==0xBB)||(doze_buf[0] ==0xCC))
				{
			...
			switch(doze_buf[0])
				{
				case 'c':
					gesture_code = KEY_GESTURE_CHAR_C;
					break;
				
				case 'm':
					gesture_code = KEY_GESTURE_CHAR_M;
					break;
					
				case 'o':
					gesture_code = KEY_GESTURE_CHAR_O;
					
					break;
					
				case 'w':
					gesture_code = KEY_GESTURE_CHAR_W;
					
					break;
					
				case 0xAA:
					gesture_code = KEY_GESTURE_RIGHT;
					
					break;
				case 0xBB:
					gesture_code = KEY_GESTURE_LEFT;
					
					break;
				case 0xCC:
					gesture_code = KEY_GESTURE_DOUBLE_TAP;
					break;
				default:
					gesture_code = KEY_GESTURE_DOUBLE_TAP;
			...
			doze_buf[0] = 0;
			gt1x_i2c_write(GTP_REG_WAKEUP_GESTURE, doze_buf, 1);
			GTP_DEBUG("Gesture code: %d,", gesture_code);

			/*input_report_key(dev, KEY_POWER, 1);
			input_sync(dev);
			input_report_key(dev, KEY_POWER, 0);
			input_sync(dev);*/
			input_report_key(dev, gesture_code, 1);
			input_sync(dev);
			input_report_key(dev, gesture_code, 0);
			input_sync(dev);
...
#ifdef CONFIG_GTP_GESTURE_WAKEUP_ZHANGZHAO
...
s32 gt1x_init_node(struct i2c_client *client)
{
#ifdef CONFIG_GTP_GESTURE_WAKEUP
	...
	if(client != NULL)
	gt1k_create_wake_gesture_sysfs(client);

	GTP_INFO("Create proc entry[GESTURE_NODE] success!");
#endif


	mt_tsc_entry = proc_create("driver/ts_information", 0644, NULL, &tsc_ops);
	if (mt_tsc_entry)
		{
       	    GTP_ERROR("create tsc_id success\n");
		}
	else
...
void gt1x_deinit_node(void)
{
	...
	gt1k_remove_wake_gesture_sysfs(gt1x_i2c_client);
#endif

	proc_remove(mt_tsc_entry);
