...
static int hall_init(struct platform_device * pdev )
{
	...
	*/	
	...
	pr_err("hall gpio_is_valid %d\n", ret);
	if (gpio_is_valid(ret)) {
		pr_err("hall irq get gpio %d\n", ret);
	...
	node=of_find_matching_node(node, hall_match_table);
	
	if (node) {
		hall_ic_data->irq =  irq_of_parse_and_map(node, 0);
		pr_err("hall irq get number %d\n", hall_ic_data->irq);
		if (!hall_ic_data->irq) {
			pr_err("HALL Failed to get irq number.");
			//goto err_get_irq;
		}
