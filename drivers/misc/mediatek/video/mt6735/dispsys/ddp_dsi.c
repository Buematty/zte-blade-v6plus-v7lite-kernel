				    ...
				    LCM_DSI_PARAMS *dsi_params)
{
	...
	bool hstx_cklp_en = true;//zhangjian modify false ->true
		...
		for (i = 0; i < 3; i++) {
			...
			if (read_data0.byte1 ==
				...
				ret = 0;	/* esd pass */
                             ...
                             printk("[DSI]enter cmp read_data0 byte0=0x%x byte1=0x%x byte2=0x%x byte3=0x%x, check_table cmd=0x%x\n",
                             read_data0.byte0, read_data0.byte1, read_data0.byte2,  read_data0.byte3,dsi_params->lcm_esd_check_table[i].cmd);
                             ret = 1;	/* esd fail */
                             break;
			}
		}
