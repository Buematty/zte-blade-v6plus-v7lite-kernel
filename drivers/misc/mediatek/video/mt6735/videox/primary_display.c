...
static int primary_display_esd_check_worker_kthread(void *data)
{
	...
	int count = 0;
    static bool esd_recovery_nedd_send_bl = false;//zhangjian add for esd
	...
	while (1) {
		...
		if (ret == 1) {
			...
			while (i--) {
                           ...
                           msleep(50);//zhangjian add for esd
                           
                           ret = primary_display_esd_check();
                           if (ret == 0) {
                               esd_recovery_nedd_send_bl = true;//zhangjian add for esd
...
#endif
//zhangjian add for esd send BL cmd
             if (esd_recovery_nedd_send_bl)
             {
                 esd_recovery_nedd_send_bl = false;                 
                 primary_display_setbacklight(100);
             }
//zhangjian add end
		if (kthread_should_stop())
			break;
