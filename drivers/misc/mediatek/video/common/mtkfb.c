...
#endif
//zhangjian add for adb read lcd info
#define PANEL_NAME_MAX_LEN 60
#define HX8392A_LD_PANEL           "hx8392a_720p_ld_dsi_vdo"     //yangchaofeng add for P635A31
#define SUMSING_S6E8_HD_VDO_PANEL           "sumsing_s6e8_hd_vdo_dsi"     //yangchaofeng add for P635A31
#define RM68200_LIDE_PANEL             "rm68200"
#define ILI9881C_DIJING_PANEL     "ili9881c_dsi_vdo_dijing_hd720"
#define ILI9881C_YASHI_PANEL     "ili9881c_dsi_vdo_yashi_hd720"
static char  lcm_proc_info[60]={"0"};
u8 lcd_read_vendor_id = 0;

static ssize_t lcd_info_read(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
    int len = 0;
    uint8_t buffer_tpd[100];
   if (!strncmp(mtkfb_lcm_name, RM68200_LIDE_PANEL, strnlen(RM68200_LIDE_PANEL, PANEL_NAME_MAX_LEN)))
        strcpy(lcm_proc_info, "IC:RM68200; Vendor:lide; Resolution:720*1280");
        else if (!strncmp(mtkfb_lcm_name, ILI9881C_DIJING_PANEL, strnlen(ILI9881C_DIJING_PANEL, PANEL_NAME_MAX_LEN)))
        strcpy(lcm_proc_info, "IC:ILI9881C; Vendor:dijing; Resolution:720*1280");        
        else if (!strncmp(mtkfb_lcm_name, ILI9881C_YASHI_PANEL, strnlen(ILI9881C_YASHI_PANEL, PANEL_NAME_MAX_LEN)))
        strcpy(lcm_proc_info, "IC:ILI9881C; Vendor:yashi; Resolution:720*1280");  
        else if (!strncmp(mtkfb_lcm_name, HX8392A_LD_PANEL, strnlen(HX8392A_LD_PANEL, PANEL_NAME_MAX_LEN)))
        strcpy(lcm_proc_info, "IC:HX8392A; Vendor:tianma; Resolution:720*1280");
        else if (!strncmp(mtkfb_lcm_name, SUMSING_S6E8_HD_VDO_PANEL, strnlen(SUMSING_S6E8_HD_VDO_PANEL, PANEL_NAME_MAX_LEN)))
        strcpy(lcm_proc_info, "IC:S6E8; Vendor:sumsing; Resolution:720*1280");
        else
		strcpy(lcm_proc_info, "0"); 
    
    len += sprintf(buffer_tpd+len, "%s\n",lcm_proc_info);
...
static const struct file_operations Mtk_LCM_info = {
        .owner = THIS_MODULE,
        .read = lcd_info_read,
        .write = NULL,
    };
int  LCM_create_proc(void)	
{
    struct proc_dir_entry *mt_entry = NULL;
    if (!strncmp(mtkfb_lcm_name, RM68200_LIDE_PANEL, strnlen(RM68200_LIDE_PANEL, PANEL_NAME_MAX_LEN)))
        lcd_read_vendor_id = 0x60;
    else if (!strncmp(mtkfb_lcm_name, ILI9881C_DIJING_PANEL, strnlen(ILI9881C_DIJING_PANEL, PANEL_NAME_MAX_LEN)))
        lcd_read_vendor_id = 0x67;  
    mt_entry = proc_create("driver/lcd_id", 0644, NULL, &Mtk_LCM_info);
    if (mt_entry)
    {
...
static int mtkfb_probe(struct device *dev)
{
	...
	MSG_FUNC_LEAVE();
    LCM_create_proc();//zhangjian add for ladb read LCD info
	return 0;
