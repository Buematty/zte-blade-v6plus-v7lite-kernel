...
#include <mt-plat/mt_gpio.h>//zhangjian add
#include "leds_sw.h"
#include "leds_hal.h"
#include "ddp_pwm.h"
#include "mtkfb.h"
//zhangjian add for led blink
#include <linux/gpio.h>
extern bool red_blink;
extern bool green_blink;
...
int mt_brightness_set_pmic(enum mt65xx_led_pmic pmic_type, u32 level, u32 div)
{
	...
	printk("zhangjianPMIC#%d:%d\n", pmic_type, level);
	mutex_lock(&leds_pmic_mutex);
	if (pmic_type == MT65XX_LED_PMIC_NLED_ISINK0) {
          //zhangjian add for led blink
          #ifdef CONFIG_P635A60
          if (level)
          {
              red_brightness = 1;                
          }
          ...
          if (red_blink)
          {
              printk("red_blink is %d",red_blink);
          ...
          if (green_blink)
          {
              printk("red_blink is %d",green_blink);
              mutex_unlock(&leds_pmic_mutex);
              return 0;
          }
          #endif
...
//zhangjian add for P635A32 GPIO control button LED
#ifdef CONFIG_P635A32
#define GPIO_BUTTON_BL_PIN      93 
static void button_setbacklight(unsigned int level)
{
	if (level == 0){
		// OFF
		gpio_set_value(GPIO_BUTTON_BL_PIN, 0);
	}else{
		// ON
		gpio_set_value(GPIO_BUTTON_BL_PIN, 1);
	}
}
#endif
unsigned int LCD_brightness_mapping(unsigned int level)
{
	...
	if (level < 200)	
	    mapped_level = 8*level/10;
	else
	    mapped_level = 8*level/10 + 2*level*(level -200)/550; //mapped_level = 0.8level + 0.2level * (level -200)/(255-200)		   
	return mapped_level;
}
//zhangjian add end
int mt_mt65xx_led_set_cust(struct cust_mt65xx_led *cust, int level)
{
	...
	case MT65XX_LED_MODE_GPIO:
              ...
              //zhangjian modify for P635A32 GPIO control button LED		
              #ifdef CONFIG_P635A32
              if((strcmp(cust->name,"button-backlight") == 0))
              {
                  printk("button-backlight is GPIO mode!!!!!\n");
                  button_setbacklight(level);
              }
              return 1;              
              #else                
	...
	case MT65XX_LED_MODE_CUST_LCM:
		if (strcmp(cust->name, "lcd-backlight") == 0)
			level = LCD_brightness_mapping(tmp_level);//zhangjian add
			bl_brightness_hal = level;
...
//zhangjian add for led blink
#ifdef CONFIG_P635A60
void red_blink_add(bool blink)
{
    if (blink)
    {
        printk("red_blink_add blink on \n");
        pmic_set_register_value(PMIC_RG_DRV_32K_CK_PDN,0x0); // Disable power down  
        pmic_set_register_value(PMIC_RG_DRV_ISINK0_CK_PDN,0x0);
        pmic_set_register_value(PMIC_RG_DRV_ISINK0_CK_CKSEL,0x0);
    ...
    {
        printk("red_blink_add blink off ,red_brightness is %d \n",red_brightness);		
        pmic_set_register_value(PMIC_RG_DRV_32K_CK_PDN, 0x0);	/* Disable power down */
        pmic_set_register_value(PMIC_RG_DRV_ISINK0_CK_PDN, 0);
        pmic_set_register_value(PMIC_RG_DRV_ISINK0_CK_CKSEL, 0);
        pmic_set_register_value(PMIC_ISINK_CH0_MODE, ISINK_PWM_MODE);
...
void green_blink_add(bool blink)
{
    if (blink)
    {
        printk("green_blink_add blink on\n");
        pmic_set_register_value(PMIC_RG_DRV_32K_CK_PDN,0x0); // Disable power down  
        pmic_set_register_value(PMIC_RG_DRV_ISINK1_CK_PDN,0x0);
        pmic_set_register_value(PMIC_RG_DRV_ISINK1_CK_CKSEL,0x0);
    ...
    {
        printk("green_blink_add blink off ,green_brightness is %d \n",green_brightness);
        pmic_set_register_value(PMIC_RG_DRV_32K_CK_PDN, 0x0);	/* Disable power down */
        pmic_set_register_value(PMIC_RG_DRV_ISINK1_CK_PDN, 0);
        pmic_set_register_value(PMIC_RG_DRV_ISINK1_CK_CKSEL, 0);
        pmic_set_register_value(PMIC_ISINK_CH1_MODE, ISINK_PWM_MODE);
