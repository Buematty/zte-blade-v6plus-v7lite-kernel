...
#include <linux/gpio.h>//zhangjian add
...
extern void red_blink_add(bool blink);
extern void green_blink_add(bool blink);
...
static ssize_t show_green_blink(struct device *dev, struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d,\n", green_blink);;
}
static ssize_t store_green_blink(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t size)
{	
    unsigned int input;    
    if (sscanf(buf, "%u", &input) != 1)
    ...
    {
        green_blink = true;
        green_blink_add(true);   
    }
    else
    {
        green_blink = false;
        green_blink_add(false); 
    }
    return size;
}
static DEVICE_ATTR(green_blink, 0664, show_green_blink, store_green_blink);
//zhangjian add end
/****************************************************************************
 * driver functions
 ***************************************************************************/
	...
	for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
		...
		if (strcmp(g_leds_data[i]->cdev.name, "lcd-backlight") == 0) {
			...
			bl_setting = &g_leds_data[i]->cust;
              ...
              rc = device_create_file(g_leds_data[i]->cdev.dev, &dev_attr_red_blink);
              }
              if (strcmp(g_leds_data[i]->cdev.name, "green") == 0) {
              rc = device_create_file(g_leds_data[i]->cdev.dev, &dev_attr_green_blink);
              }
              //zhangjian end add
		if (ret)
			goto err;
...
/* platform_device_unregister(&mt65xx_leds_device); */
	...
	#ifdef CONFIG_P635A60 //zhangjian add for A60
    gpio_set_value(59, 1);
	#endif
