...
static bool OV13853_read_eeprom(kal_uint16 addr, BYTE* data, kal_uint32 size ){
	...
	//int offset = addr;
	int offset =0;
	OV13853_selective_read_eeprom(0x0779, &flag1);
	printk("--zlx--OV13853_read_eeprom: flag1=%d\n", flag1);
	offset = 0x077A;
	{
		for(i = 0; i < 496; i++) {
			...
			printk("read_eeprom 0x%0x 0x%x\n",offset, data[i]);
			offset++;
		}
	}
	OV13853_selective_read_eeprom(0x096B, &flag2);
	printk("--zlx--OV13853_read_eeprom: flag2=%d\n", flag2);
	offset=0x96C;
	{
		for(i = 496; i < 1372; i++) {
		if(!OV13853_selective_read_eeprom(offset, &data[i])){
