...
#define OV8855R2AOTP 1

/* #define LOG_WRN(format, args...) xlog_printk(ANDROID_LOG_WARN ,PFX, "[%S] " format, __FUNCTION__, ##args) */
/* #defineLOG_INF(format, args...) xlog_printk(ANDROID_LOG_INFO ,PFX, "[%s] " format, __FUNCTION__, ##args) */
/* #define LOG_DBG(format, args...) xlog_printk(ANDROID_LOG_DEBUG ,PFX, "[%S] " format, __FUNCTION__, ##args) */
#define LOG_INF(format, args...)    pr_debug(PFX "[%s] " format, __func__, ##args)
//extern void mtk_sensorinfo_set_front_sensor_id(uint16_t id);
typedef enum {
	OV8855R2A,
	OV8855R1A
} OV8855_VERSION;

OV8855_VERSION ov8855version = OV8855R2A;
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {
				//mtk_sensorinfo_set_front_sensor_id(OV8855_SENSOR_ID);
					LOG_INF("OV8855 i2c write id: 0x%x, sensor id: 0x%x\n",imgsensor.i2c_write_id, *sensor_id);
					return ERROR_NONE;
			}
...
#if 0
UINT32 OV8855R2A_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
