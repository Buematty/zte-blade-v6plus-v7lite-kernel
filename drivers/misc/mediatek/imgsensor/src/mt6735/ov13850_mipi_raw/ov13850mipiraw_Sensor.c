...
static void set_mirror_flip(kal_uint8 image_mirror)
{
	...
	else if(image_mirror == 3)
	{
		image_mirror = 0;
	}
	else if(image_mirror == 1)
	{
		image_mirror = 2;
	}
	else if(image_mirror == 2)
	{
		image_mirror = 1;
	}
	else
        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("LHL--13850--OK--i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            LOG_INF("---LHL-13850--ERROR--Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
static kal_uint32 open(void)
{
    ...
    LOG_INF("----before sensor_init zx!---- \n");
    sensor_init();
	  //for OTP
	  LOG_INF("----after sensor_init zx!---- \n");
	  otp_cali(imgsensor.i2c_write_id);
	  LOG_INF("----after otp_cali zx!---- \n");
	  write_cmos_sensor(0x0100, 0x00);
