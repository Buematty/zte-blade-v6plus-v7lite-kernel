...
#include "sp2510mipi_Sensor.h"

#ifdef CONFIG_HW_INFO
#include <linux/hw_info.h>
static struct hw_info sub_camera =
{
.hst = SUB_CAM,
.name = "sp2509_2M",
.vendor = ""
...
#define PFX "SP2510_camera_sensor"
...
{	
LOG_INF("image_mirror = %d\n", image_mirror);		    
write_cmos_sensor(0xfd,0x01);	 //page set
switch (image_mirror)	{		
...
static void sensor_init(void)
{
      ...
      write_cmos_sensor(0xfb,0x73);//9.29canshu 63
			...
			write_cmos_sensor(0xa0,0x05);//09 mipi phase
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		if(!camera_gpio){
			...
			*sensor_id = SP2510_SENSOR_ID;
			LOG_INF("vendor_id:microkore\n");	
		}
		#ifdef CONFIG_HW_INFO
	    strcpy(sub_camera.vendor,"MicroKore");
		hw_info_add(&sub_camera);
		#endif		   
                return ERROR_NONE;
            }
...
UINT32 SP2510_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
