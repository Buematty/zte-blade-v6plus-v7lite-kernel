...
static struct hw_info main_camera =
{
.hst = MAIN_CAM,
.name = "gc5005_5M",
.vendor = ""
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = 0x5005,        //record sensor id defined in Kd_imgsensor.h

    .checksum_value = 0xf7375923,        //checksum value for Camera Auto Test

    .pre = {
        .pclk = 58500000,//54000000,                //record different mode's pclk
        .linelength = 914,//906,                //record different mode's linelength
...
/*SYS*/
	...
	write_cmos_sensor(0x06, 0xc9); //c5                                                                                                        
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		if(camera_gpio){
			...
			*sensor_id = GC5005_MIPI_SENSOR_ID;
			LOG_INF("vendor_id:0x%x\n", *sensor_id);	
		}
		#ifdef CONFIG_HW_INFO
	    strcpy(main_camera.vendor,"SunWinTech");
		hw_info_add(&main_camera);
		#endif		   
                return ERROR_NONE;
            }
