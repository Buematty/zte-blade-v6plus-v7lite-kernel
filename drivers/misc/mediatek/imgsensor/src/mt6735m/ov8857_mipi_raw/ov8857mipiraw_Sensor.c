...
#include "ov8857mipiraw_Sensor.h"

#define PFX "ov8857"

#define ov8857R1AOTP 1
...
ov8857_VERSION ov8857version = ov8857R1A;

static DEFINE_SPINLOCK(imgsensor_drv_lock);

static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = OV8857_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
{
	kal_uint32 RealSensorId=0;
	RealSensorId=((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
	if(RealSensorId == OV8856_SENSOR_ID)
		{
          LOG_INF("Read OV8857 Sensor ID succeed!!!!");
		  RealSensorId = RealSensorId | 0x0001;
	    }
	return RealSensorId;
}
...
static void sensor_init(void)
{
    ...
    */
	...
	write_cmos_sensor(0x376b,0x36);//0x30;
	...
	write_cmos_sensor(0x3820,0xc6);//0x80;
	write_cmos_sensor(0x3821,0x00);//,0x46;
	...
	write_cmos_sensor(0x5001,0x0e);//0x0a;
	write_cmos_sensor(0x5004,0x00);//0x04;
	write_cmos_sensor(0x502e,0x00);//0x03;
...
static void preview_setting(void)
{
	...
	write_cmos_sensor(0x3818,0x00);
	write_cmos_sensor(0x3819,0x00);
	write_cmos_sensor(0x3820,0xd6);//0x90;
	write_cmos_sensor(0x3821,0x21);//0x67;
		...
		do {
            *sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("ov8857 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				return ERROR_NONE;
				}
            LOG_INF("Read ov8857 id fail, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
{
	int RG_Ratio_Typical = 0x14e, BG_Ratio_Typical = 0x161;
...
UINT32 OV8857_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
