/*
 * BU64295AF voice coil motor driver
 *
 *
 */
...
#define AF_DRVNAME "BU64295AF_DRV"
...
static int s4AF_ReadReg(unsigned short *a_pu2Result)
{
    ...
    *a_pu2Result = (((u16)pBuff[0] & 0x03) << 8) + (pBuff[1]);	// pBuff[0] = PS EN W2 W1 W0 M D9 D8; pBuff[1] = D[7:0]
	return 0;
}
...
static int s4AF_WriteReg(u16 a_u2Data)
{
	...
	#endif
	LOG_INF("gaoyangyang add Init Pos %ld\n", (unsigned long)puSendCmd[0]);
	LOG_INF("gaoyangyang add Init Pos %ld\n", (unsigned long)puSendCmd[1]);
...
static int init_setting(void)
{
	int  i4RetValue;
	char puSendCmd[2];
	char PS, EN, Wx, Mode, Qfact, MSB, resonant_frequency,Xmode;
	resonant_frequency = 123; /* 79HZ */
	Xmode = 1; /* 0~2  0 0.5x mode ;1 0.8x mode;2 1.0x mode */
...
/* ////////////////////////////////////////////////////////////// */
long BU64295AF_Ioctl(struct file *a_pstFile, unsigned int a_u4Command, unsigned long a_u4Param)
{
	long i4RetValue = 0;
...
int BU64295AF_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
	LOG_INF("Start\n");
...
void BU64295AF_SetI2Cclient(struct i2c_client *pstAF_I2Cclient, spinlock_t *pAF_SpinLock, int *pAF_Opened)
{
