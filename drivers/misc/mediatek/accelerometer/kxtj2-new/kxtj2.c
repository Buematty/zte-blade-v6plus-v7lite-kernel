...
/*Kionix Auto-Cali Start*/
		...
		if((abs(raw[0]) < Detection_range)  &&  (abs(raw[1]) < Detection_range) 
...
{   /*write anything to this register will trigger the process*/
	struct item{s16 raw[KXTJ2_1009_AXES_NUM];};
	struct i2c_client *client = kxtj2_1009_i2c_client;  
	int res, num;
...
static int kx_test_poll_thread(void *data)
{
	...
	while (1) 
	{
		wait_event_interruptible(test_data.wq, ((test_data.wkp_flag != 0) || kthread_should_stop()));
...
static ssize_t kx_poll_test(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if (test_data.init_done == false) {
		...
		test_data.wkp_flag = 0;
		test_data.task = kthread_run(kx_test_poll_thread, &test_data, "kionix_test");
...
static const struct i2c_device_id kxtj2_1009_i2c_id[] = 
{
	{KXTJ2_1009_DEV_NAME,0},
	{}
};
...
{
	const char *name = "mediatek,kxtj2";
