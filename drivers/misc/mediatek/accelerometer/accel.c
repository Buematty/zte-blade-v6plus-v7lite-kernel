...
#include "inc/accel_calibration.h" // lijiangshuo add for acc calibration 20160105

#define ACC_CAL_NUM	10 // lijiangshuo add for acc calibration 20160105
#define ACC_CAL_RANGE	3920 // 400mg:400*9.8=3920
...
/* lijiangshuo add for accel calibration 20160105 start */
			x -= acc_context_obj->cali_sw[ACC_AXIS_X];
			y -= acc_context_obj->cali_sw[ACC_AXIS_Y];
			z -= acc_context_obj->cali_sw[ACC_AXIS_Z];
/* lijiangshuo add for accel calibration 20160105 start */
...
static struct acc_context *acc_context_alloc_object(void)
{
	...
	obj->cali_sw[ACC_CAL_RESULT] = -1; // lijiangshuo add for acc calibration 20160105
	ACC_LOG("acc_context_alloc_object----\n");
	return obj;
}
...
int acc_calibration(int type, int cali[3])
{
	...
	int x, y, z, status, bias_tmp[3] = {0};
	int cal_valid = 0, flag = 0, i = ACC_CAL_NUM;
	int pre_enable = 0;
	...
	for (; i > 0; i--){		
		...
		ACC_LOG("acc: calibrate get data %d: [%d, %d %d]\n",i, x, y, z);
		bias_tmp[0] += x;
		bias_tmp[1] += y;
		bias_tmp[2] += z;		
		mdelay(10);
	...
	bias_tmp[0] = bias_tmp[0] / ACC_CAL_NUM;
	bias_tmp[1] = bias_tmp[1] / ACC_CAL_NUM;
	bias_tmp[2] = bias_tmp[2] / ACC_CAL_NUM;

	if (bias_tmp[0] > ACC_CAL_RANGE || bias_tmp[0] < -ACC_CAL_RANGE || bias_tmp[1] > ACC_CAL_RANGE || bias_tmp[1] < -ACC_CAL_RANGE || bias_tmp[2] > (GRAVITY_EARTH_1000+ACC_CAL_RANGE) || bias_tmp[2] < (GRAVITY_EARTH_1000-ACC_CAL_RANGE)) {
		ACC_ERR("acc: invalid calibration data!\n");
		goto out;
	}

	cal_valid = 1;
	cxt->cali_sw[ACC_AXIS_X] = bias_tmp[0];
	cxt->cali_sw[ACC_AXIS_Y] = bias_tmp[1];
	cxt->cali_sw[ACC_AXIS_Z] = bias_tmp[2] - GRAVITY_EARTH_1000;
	cxt->cali_sw[ACC_CAL_RESULT] = cal_valid; // 0:invalid(failed); 1:valid(success)
	cali[0] = cxt->cali_sw[ACC_AXIS_X];
	cali[1] = cxt->cali_sw[ACC_AXIS_Y];
	cali[2] = cxt->cali_sw[ACC_AXIS_Z];

out:
	if ((flag == 1) && (pre_enable == 0)) {
		cxt->is_active_nodata = false;
		acc_real_enable(0);
	}

	ACC_LOG("acc cal valid :%d, cal_params [%d, %d, %d]\n", cal_valid, cali[0], cali[1], cali[2]);
...
/* lijiangshuo add for accel calibration 20160105 start */
static ssize_t acc_show_calibrate(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t ret;

	ret = sprintf(buf, "%d %d %d %d\n", acc_context_obj->cali_sw[ACC_CAL_RESULT], 
		acc_context_obj->cali_sw[ACC_AXIS_X], acc_context_obj->cali_sw[ACC_AXIS_Y], acc_context_obj->cali_sw[ACC_AXIS_Z]);

	return ret;
}

// clear acc calibration params acctually
static ssize_t acc_store_calibrate(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{	
	acc_context_obj->cali_sw[ACC_AXIS_X] = 0;
	acc_context_obj->cali_sw[ACC_AXIS_Y] = 0;
	acc_context_obj->cali_sw[ACC_AXIS_Z] = 0;
	acc_context_obj->cali_sw[ACC_CAL_RESULT] = -1;

	return size;
}
/* lijiangshuo add for accel calibration 20160105 end */
...
/* lijiangshuo add for accel calibration 20160105 start */
DEVICE_ATTR(acccalibrate, S_IWUSR | S_IRUGO, acc_show_calibrate, acc_store_calibrate);
/* lijiangshuo add for accel calibration 20160105 end */
...
//	cxt->acc_ctl.acc_calibration = ctl->acc_calibration;
	cxt->acc_ctl.acc_calibration = acc_calibration; // lijiangshuo modified for acc calibration 20160105
...
/* lijiangshuo add for acc calibration 20160105 start */
	err = acc_calibration_init();
	if(err)
		ACC_ERR("acc calibration init failed!\n");
/* lijiangshuo add for acc calibration 20160105 end */
