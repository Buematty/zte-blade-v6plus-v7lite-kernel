...
/* lijiangshuo modify temp for pv light sensor not sensitive 20160130 start */		
		if(mdelay >= 500)
			mdelay = 200;
/* lijiangshuo modify temp for pv light sensor not sensitive 20160130 end */		
...
static ssize_t ps_show_factory_cal_value(struct device *dev, struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", alsps_context_obj->alsps_factory_calibration_value);
}
...
DEVICE_ATTR(psfactorycalvalue, S_IWUSR | S_IRUGO, ps_show_factory_cal_value, NULL); // lijiangshuo add for ps calibration 20160104
...
static struct attribute *alsps_attributes[] = {
	...
	&dev_attr_psfactorycalvalue.attr, // lijiangshuo add for ps calibration 20160104
	NULL
};
...
/* lijiangshuo add for ps calibration 20160104 start */
	err = alsps_calibration_init();
	if(err)
		ALSPS_ERR("alsps calibration init failed!\n");
/* lijiangshuo add for ps calibration 20160104 end */
