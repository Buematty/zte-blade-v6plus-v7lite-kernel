...
static u16 ltr559_ps_get_calibration_crosstalk(void)
{
	...
	struct i2c_client *client = ltr559_obj->client;
	u8 regdata1 = 0, regdata2 = 0, regdata3 = 0, regdata = 0;	
	...
	regdata3 = ltr559_i2c_read_reg(LTR559_INTERRUPT);
	ret  = ltr559_i2c_write_reg(LTR559_INTERRUPT, 0x0);
	if(ret < 0)
	{
		APS_ERR("PS: ps_calibration set interrupt failed!\n");
		return ret;
	}
	...
	ret = ltr559_i2c_write_reg(LTR559_INTERRUPT, regdata3);
	if(ret < 0)
	{
		APS_ERR("PS: ps_calibration set interrupt back failed!\n");
		return ret;
	}
...
static void ltr559_ps_check_calibration_result(u16 ps_avg, u16 value, int type)
{
	...
	case PS_CAL_STARTUP: // startup cal, compare ps_avg with ps_factory_cal_value
		if(ps_avg > value)
		//if((ps_avg > value)&&(ps_avg - value) > 2047) // TODO
		{
			ps_cali.valid = 0;
			...
			//APS_LOG("%s ps_avg %d exceed the factory cal value %d too much!\n",
				//__func__, ps_avg, value);
		}
		break;
...
static int ltr559_devinit(void)
{
	...
	res = ltr559_i2c_write_reg(LTR559_PS_N_PULSES, 1); 
	if(res < 0)
	{
...
static void ltr559_eint_work(struct work_struct *work)
{
	...
	else
	{
		...
		if(intr_flag_value){
			APS_DBG("%s ps_data(%d) > ps_threhold_high(%d), report NEAR!\n", __func__, obj->ps, obj->ps_thd_val_high);
			databuf[0] = LTR559_PS_THRES_LOW_0;	
			databuf[1] = (u8)((obj->ps_thd_val_low) & 0x00FF);
			...
			APS_DBG("%s ps_data(%d) < ps_threhold_low(%d), report FAR!\n", __func__, obj->ps, obj->ps_thd_val_low);
			databuf[0] = LTR559_PS_THRES_LOW_0;	
