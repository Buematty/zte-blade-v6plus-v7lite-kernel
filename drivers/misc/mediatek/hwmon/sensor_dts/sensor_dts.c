...
struct alsps_hw *get_alsps_dts_func(const char *name, struct alsps_hw *hw)
{
	...
	u32 ps_threshold_high_def[] = {0};
	u32 ps_threshold_low_def[] = {0};
	u32 ps_power_on_max_ct[] = {0};
	u32 ps_3cm_delta[] = {0};
	u32 ps_5cm_delta[] = {0};
	...
	ret = of_property_read_u32_array(node , "ps_threshold_high_def", ps_threshold_high_def, ARRAY_SIZE(ps_threshold_high_def));
	if (ret == 0)
		hw->ps_threshold_high_def	 = ps_threshold_high_def[0];

	ret = of_property_read_u32_array(node , "ps_threshold_low_def", ps_threshold_low_def, ARRAY_SIZE(ps_threshold_low_def));
	if (ret == 0)
		hw->ps_threshold_low_def	 = ps_threshold_low_def[0];

	ret = of_property_read_u32_array(node , "ps_power_on_max_ct", ps_power_on_max_ct, ARRAY_SIZE(ps_power_on_max_ct));
	if (ret == 0)
		hw->ps_power_on_max_ct	 = ps_power_on_max_ct[0];

	ret = of_property_read_u32_array(node , "ps_3cm_delta", ps_3cm_delta, ARRAY_SIZE(ps_3cm_delta));
	if (ret == 0)
		hw->ps_3cm_delta		 = ps_3cm_delta[0];

	ret = of_property_read_u32_array(node , "ps_5cm_delta", ps_5cm_delta, ARRAY_SIZE(ps_5cm_delta));
	if (ret == 0)
		hw->ps_5cm_delta		 = ps_5cm_delta[0];
