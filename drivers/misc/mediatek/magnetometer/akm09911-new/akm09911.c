...
/* lijiangshuo add for AKM GR 20151224 start */
#define AK0991X_WIA_SIZE	4
#define AK0991X_FUSE_SIZE	3
#define AK0991X_INFO_SIZE (AK0991X_WIA_SIZE + AK0991X_FUSE_SIZE)
static unsigned char sense_info[AKM_SENSOR_INFO_SIZE];
static unsigned char sense_conf[AKM_SENSOR_CONF_SIZE];
/* lijiangshuo add for AKM GR 20151224 end */
...
/* lijiangshuo add for AKM GR 20151224 start */
static ssize_t bin_attr_info_read( struct file *file, struct kobject *kobj, struct bin_attribute *attr,
	char *buf, loff_t pos, size_t size)
{
	...
	memcpy(&info[AK0991X_WIA_SIZE], sense_conf, AK0991X_FUSE_SIZE);
...
/* lijiangshuo add for AKM GR 20151224 start */
static struct bin_attribute akm09911_bin_attributes[] = {
	__BIN_ATTR(info, 0444, bin_attr_info_read, NULL, AK0991X_INFO_SIZE),
	__BIN_ATTR_NULL
};
/* lijiangshuo add for AKM GR 20151224 end */
...
static int akm09911_create_attr(struct device_driver *driver)
{
	...
	/* lijiangshuo add for AKM GR 20151224 start */
	compass = class_create(THIS_MODULE, AKM_SYSCLS_NAME);
	if (IS_ERR(compass)) {
		err = PTR_ERR(compass);
		return err;
	}
	
	class_dev = device_create(compass, NULL, akm_compass_device_dev_t, NULL, AKM_SYSDEV_NAME);
	if (IS_ERR(class_dev)) {
		err = PTR_ERR(class_dev);
		return err;
	}

	err = sysfs_create_bin_file(&class_dev->kobj, &akm09911_bin_attributes[0]);
	if(err)
		return err;
	/* lijiangshuo add for AKM GR 20151224 end */
...
static long akm09911_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	uint32_t enable;
/* lijiangshuo del for AKM GR 20151224 start */
...
/* lijiangshuo del for AKM GR 20151224 end */
